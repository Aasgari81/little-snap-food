...
class Ui_editmenuMainWindow(object):
    def setupUi(self, MainWindow, food_id):
...
"    }\n"
"    ")
        self.food_id = food_id # food id that we want to edit
        ...
        self.picture_path_input.setText("")
...
"")
        ...
        self.submit_btn.clicked.connect(self.edit_menu)


        sql = "SELECT * FROM foodofrestuarant WHERE food_id = %s"
        val = (self.food_id,)
        mycursor.execute(sql, val)
        myresult = mycursor.fetchall()
        for info in myresult:
            # give a default value to line edites 
            self.first_name_input.setText(str(info[3]))
            self.last_name_input.setText(str(info[5]))
            self.phonenumber_input.setText(str(info[6]))
            self.email_input.setText(str(info[7]))
            self.picture_path_input.setText(str(info[4]))
    ...
    def edit_menu(self):
        """edit food in database by clicking on a button"""
        food_id = int(self.food_id)
        if self.first_name_input.text().strip() != '' and self.last_name_input.text().strip() != '' and self.phonenumber_input.text().strip() != '' and self.email_input.text().strip() != '' and self.picture_path_input.text().strip() != '': # check that all line edites don't be empty
            sql = "UPDATE foodofrestuarant SET food_name = %s, food_material = %s, food_fixedprice = %s, food_customerprice = %s, food_inventory = %s WHERE food_id = %s"
            val = (self.first_name_input.text(), self.last_name_input.text(), self.phonenumber_input.text(), self.email_input.text(), self.picture_path_input.text(),food_id)
            mycursor.execute(sql, val)
            mydb.commit()
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_6.setText(_translate("MainWindow", "material: "))
        self.label_2.setText(_translate("MainWindow", "fixed price: "))
        self.label_3.setText(_translate("MainWindow", "customer price: "))
        self.label_10.setText(_translate("MainWindow", "inventory: "))
